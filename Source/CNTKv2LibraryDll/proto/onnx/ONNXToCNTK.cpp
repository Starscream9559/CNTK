...
private:
    ...
    static std::pair<std::vector<size_t>, std::vector<size_t>> CalcPaddingForSameLowerOrUpperAutoPad(
        const Variable &input, const NDShape& kernelShape, const NDShape& strides, bool isSameLower);
    //
    ...
    //
    static std::pair<std::vector<size_t>, std::vector<size_t>> ONNXToCNTKHelper::CalcPaddingFromOutputShape(
    ...
    static FunctionPtr CreatePadOpForSameLowerOrUpperAutoPad(
        const Variable &input, const NDShape& kernelShape, const NDShape& strides, const double padValue, bool isSameLower);
...
FunctionPtr ONNXToCNTKHelper::CreateFunction(const Node *node, const std::vector<Variable> &inputs, const Graph *graph, const Variable& inputPlaceholder)
{
    ...
    else if (onnxOpName == "AveragePool" || onnxOpName == "MaxPool")
    {
        ...
        if (hasAutoPad)
        {
            ...
            {
                const bool isSameLower = auto_pad == ConvAutoPadType::SAME_LOWER;
                padsPair = CalcPaddingForSameLowerOrUpperAutoPad(inputOperand0, poolingWindowShape, strides, /*isSameLower=*/isSameLower);
                                                             ...
                                                             NDShape &strides, const Node *node, const Variable& dataOperand, const double padValue)
{
    ...
    else if (hasAutoPad)
    {
        ...
        case ConvAutoPadType::SAME_UPPER:
        {
            ...
            convOperand = (Variable) CreatePadOpForSameLowerOrUpperAutoPad(dataOperand, kernelShape, strides, padValue, /*isSameLower=*/isSameLower);
            break;
        }
        case ConvAutoPadType::VALID:
            cntkConvAutoPadding.insert(cntkConvAutoPadding.begin(), strides.Rank(), false);
    ...
    const Variable &input, const NDShape& kernelShape, const NDShape& strides, const double padValue, bool isSameLower)
{
    ...
    std::tie<std::vector<size_t>, std::vector<size_t>>(begins, ends) = CalcPaddingForSameLowerOrUpperAutoPad(input, kernelShape, strides, isSameLower);
